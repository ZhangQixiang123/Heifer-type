 ...
 */
function serializeNode(node: ts.Node, sourceFile: ts.SourceFile): any {
  const kind = ts.SyntaxKind[node.kind];
  const result: any = { kind };
  ...
  switch (node.kind) {
    ...
    case ts.SyntaxKind.VariableDeclarationList:
      const declList = node as ts.VariableDeclarationList;
 ...
 */
function parseTypeScript(fileName: string): any {
